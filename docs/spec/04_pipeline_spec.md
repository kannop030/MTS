...
入力ファイル判定
    ...
    ├─ 音声ファイル ──────────┤
    │                        ▼
    │                  ステップ2: 文字起こし（Whisper）【必須】
    │                        │
    ├─ 動画 かつ              │
    │  enable_ocr=true ──►   ステップ3: フレーム抽出      【任意】
    │                        │
    │                  ステップ4: スライド検出（PySceneDetect）
    │                        │
    │                  ステップ5: OCR（EasyOCR）
    │                        │
    ├─ enable_minutes=true ──►ステップ6: 要約生成（Ollama）【任意】
    │                        │
    │                  ステップ7: 議事録生成（Ollama）
    │                        │
    └───────────────────────►出力
...
> - **文字起こし（ステップ2）は常に実行されます。**
> - **OCR（ステップ3〜5）** は `enable_ocr=true` かつ動画ファイルの場合のみ実行されます。
> - **要約・議事録（ステップ6〜7）** は `enable_minutes=true` の場合のみ実行されます。
