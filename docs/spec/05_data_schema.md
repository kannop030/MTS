...
│
├── watch/                                             # フォルダ監視モードの入力フォルダ
│   ├── {input_file}                                   # ここにファイルを置く（処理完了後に自動削除）
│   └── failed/
│       └── {input_file}                               # 処理失敗したファイル
│
├── outputs/
│   └── {job_id}/
...
|----------|---------------|---------------|
| `uploads/{job_id}/` | Web UIアップロード時 | **処理正常終了後に自動削除** |
| `watch/{input_file}` | ユーザーがファイルを配置 | **処理正常終了後に自動削除** |
| `watch/failed/{input_file}` | フォルダ監視モードで処理失敗時 | ユーザーが明示的に削除するまで保持 |
| `temp/{job_id}/` | 処理開始時 | 処理終了後（成功・失敗問わず）自動削除 |
| `outputs/{job_id}/` | 処理完了時 | ユーザーが明示的に削除するまで保持 |

> Web UI モード: アップロードファイルは処理が正常完了した時点で即座に削除されます。エラー終了時はデバッグのため保持し、ユーザー操作で削除します。  
> フォルダ監視モード: 処理が正常完了したファイルは自動削除されます。失敗したファイルは `watch/failed/` へ移動して保持されます。
